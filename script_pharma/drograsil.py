...
def check_drogasil(cep, produto):
    ...
    try:
            ...
            # Verifica se deu erro HTTP (bloqueio 403, etc)
            if response_price.status_code != 200:
                print(f"Drogasil bloqueou ou erro HTTP: {response_price.status_code}")
                return None
                
            resp_price = response_price.json()
            ...
            if data_freight and data_freight.get("getNearbyStockByZipCode"):
                for descriptions in data_freight["getNearbyStockByZipCode"]:
                    ...
                    # AQUI: Removi a verificação de quantity > 0 para debug, mas mantive stocks
                    if stocks: 
                        product_description["estoque"] = int(stocks[0].get("quantity", 0))
        ...
        # Esse print vai mostrar o erro real sem quebrar o JSON da API
        print(f"Erro seguro Drogasil: {e}")
        return None
